...
int main(int argc, char *argv[])
{
  ...
  while (runTime->run())
  {
    ...
    for ( pimple->start(); pimple->loop(); (*pimple)++ )
    {
      ...
      smart_tmp< volScalarField > rAU;
      fvVectorMatrixHolder UEqn = fun_Ueqn( pimple, rho, p, U, phi, turbulence, rAU );
      ...
      {
        fun_pEqn( mesh, runTime, pimple, pThermo, rho, p, h, psi, U, phi, turbulence, UEqn, rAU, DpDt, cumulativeContErr, corr, rhoMax, rhoMin );
      }
