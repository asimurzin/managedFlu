...
//---------------------------------------------------------------------------
void fun_correctPhi( const TimeHolder& runTime, const fvMeshHolder& mesh,
...
//---------------------------------------------------------------------------
void readControls( const TimeHolder& runTime, const pimpleControlHolder& pimple, 
                   bool& adjustTimeStep, scalar& maxCo, scalar& maxDeltaT, bool& correctPhi, bool& checkMeshCourantNo )
{
  readTimeControls( runTime, adjustTimeStep, maxCo, maxDeltaT );

  correctPhi = pimple->dict().lookupOrDefault<Switch>("correctPhi", true);
  checkMeshCourantNo = pimple->dict().lookupOrDefault<Switch>("checkMeshCourantNo", false);
...
int main(int argc, char *argv[])
{
  ...
  fun_correctPhi( runTime, mesh, pimple, p_rgh, U, phi, rho, cumulativeContErr, pRefCell, pRefValue );
  ...
  while (runTime->run())
  {
    bool correctPhi; bool checkMeshCourantNo;
    readControls( runTime, pimple, adjustTimeStep, maxCo, maxDeltaT, correctPhi, checkMeshCourantNo );
    ...
    if ( mesh->changing() )
    {
      Info<< "Execution time for mesh.update() = " << runTime->elapsedCpuTime() - timeBeforeMeshUpdate << " s" << endl;
